...
func set_a_property(
	...
	if self.subs.has('set_a_property'):
		self.conductor_conductable.sub('set_a_property', [ new_property ])
		return

	new_property = self.conductor_conductable.filter('new_property', new_property, [ true ])

	self.conductor_conductable.hook('pre_set_a_property', [ new_property ])

	var old_a_property = self.a_property

	a_property = new_property

	self.conductor_conductable.hook('post_set_a_property', [ a_property, old_a_property ])


func get_a_property() -> String:

	if self.subs and self.subs.has('get_a_property'):
		return self.conductor_conductable.sub('get_a_property')

	self.conductor_conductable.hook('get_a_property')

	return self.conductor_conductable.filter('a_property', a_property)
